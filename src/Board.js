...
export const Board = (props) => {
  ...
  const oriBoard = [...Array(NUM_PIECES).keys()];
  ...
  const isBesideEmpty = (board, pos) => {
    const emptySquare = board.indexOf(NUM_PIECES - 1);
  ...
  const changeBoard = (board, pos) => {
    const newEmptySquare = board.indexOf(NUM_PIECES - 1);
    const index = board[pos];
    board[pos] = NUM_PIECES - 1;
  ...
  const initRandonBoard = () => {
    let newBoard = [...Array(NUM_PIECES).keys()];
